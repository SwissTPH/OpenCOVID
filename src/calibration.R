...
run_calibration = function(o) {
  ...
  o$emulator = emulator[c("model", "center", "scale")]
  
  # TODO: The calibration_df from come from the emulator file not current state of o list...
  
  # Calibration dataframe - transform parameter bounds (do initial point when initialising MCMC)
  param_df = o$calibration_df %>%
  ...
  for (canton in o$cantons) {
    ...
    canton_idx    = o$calibration_df$scope %in% c("global", canton)
    canton_params = o$calibration_df$names[canton_idx]
    ...
    canton_file    = paste0(o$pth$fitting, canton, ".rds")
    canton_archive = paste0(o$pth$archive, canton, "_", time_stamp, ".rds")
  ...
  # TODO: Use all seeds from best sample to generate uncertainty bounds
  ...
  # TODO: Also store options here... these should come from emulator
...
run_mcmc_chain = function(o, param_df, n_params, best_simulated, chain_id = NA) {
  ...
  # TODO: Perhaps best default would be upper confidenbce bound of emulator rather than prior means 
...
likelihood_function = function(parTab, data, PRIOR_FUNC) {
  ...
  likelihood_emulator = function(param_set) {
    ...
    # TODO: We can either apply the prior contribution to the likelihood value when fitting 
    #       the emulator or when running the MCMC. It depends how well the emulator performs.
    
    # TODO: Check this still works (will need o$prior2data_weights) and that < 0
...
# ---------------------------------------------------------
prior_function = function(param_set, o) {
  
  browser() # This function needs to be updated - remove hyper priors and prior_dist
  
  # Indices of hyper parameters and their dependants
  is_hyper  = o$calibration_df$scope == "hyper"
  has_hyper = o$calibration_df$hyper & !is_hyper
  
  # Simplify this process if running just one canton
  if (length(o$cantons) == 1) has_hyper[] = FALSE
  ...
  for (i in 1 : nrow(o$calibration_df) ) {
    param = o$calibration_df[i, ]
    
    # Skip those without prior, and those with hyper prior
    if ((tolower(param$prior_dist) != "none") & has_hyper[i] == 0) {
      
      # Extract prior distribution function
      prior_dist = get(param$prior_dist)
      prior_val  = prior_dist(param_set[i], 
                              param$values, 
                              param$prior_param, 
                              log = TRUE)
      
      # Prior weights (as a function of # data points x personal weighting)
      prior_weight = o$prior2data_weights[param$global + 1]
      prior_weight = prior_weight * param$prior_weight
      
      # Diffuse prior distribution for this parameter
      prior_vals = prior_vals + (prior_val * prior_weight)
    }
  }
  
  # ---- Now consider has_hyper priors ----
  
  # Skip this step if no hyper params (the case if running just one canton)
  if (any(is_hyper)) {
    
    # Convert from logical to integer to associate IDs
    is_hyper_id  = which(is_hyper)[seq(1, sum(is_hyper), by = 2)]
    has_hyper_id = o$calibration_df$hyper[!is_hyper]
    
    # Loop through the parameters that are hierarchical
    for (i in seq_len(length(is_hyper_id))) {
      hyper_param = o$calibration_df[is_hyper_id[i], ]
      
      # Get hyperprior mean and std dev
      hyper_mu = param_set[is_hyper_id[i]]
      hyper_sd = param_set[is_hyper_id[i] + 1]
      
      # Values of hierarchical parameters
      has_hyper_values = param_set[which(has_hyper_id == i)]
      
      # Get the hyperprior shape = normal prior shape
      prior_dist = get(hyper_param$prior_dist)
      prior_val  = prior_dist(has_hyper_values, 
                              hyper_mu, 
                              hyper_sd, 
                              log = TRUE)
      
      # Prior weights (as a function of # data points x personal weighting)
      prior_weight = o$prior2data_weights[1] * hyper_param$prior_weight
      
      # We can also give hyperpriors extra power - idea being to increase learning across cantons
      prior_weight = prior_weight * o$hyperprior_weight
      
      # Summing here because more than 1 log-density
      prior_vals = prior_vals + (sum(prior_val) * prior_weight)
    }
  }
