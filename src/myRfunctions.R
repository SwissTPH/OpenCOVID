...
# ---------------------------------------------------------
# Wrapper for cumsum with na.rm functionality
# ---------------------------------------------------------
cum_sum = function(x, na.rm = FALSE) {
  if (na.rm) y = cumsum(ifelse(is.na(x), 0, x)) + x*0
  else y = cumsum(x)
}
...
# ---------------------------------------------------------
try_load = function(pth, file, msg = NULL, type = "rds", sep = FALSE) {
  ...
  } else {  # Throw a descriptive error if this file doesn't exist
