...
model = function(o, p, seed = NA, short_run = FALSE, do_plot = FALSE, verbose = "date") {
  ...
  # Several elements of p still need to be parsed - this allows contol when defining alternative scenarios
  p = parse_parameters(o, p, network = network)
  ...
  for (i in p$import_date_initial : run_time) {
    ...
    pop_susceptibility = sum(1 - susceptible_immunity)
    m = update_output(o, p, m, i, "pop_susceptibility", pop_susceptibility)
    
    # Prevalence across whole population (proportion currently infected)
    pop_prevalence = 100 * n_infected / p$population_size
    m = update_output(o, p, m, i, "pop_prevalence", pop_prevalence)
    ...
    infectious_contacts = network$all_contacts %>%
      slice_sample(prop = 1 - p$npi_level[i]) %>%
    ...
    #  4) some asymptomatic & mild cases through mass testing
    
    # Define which people to test and diagnose
    to_diagnose = fn_test_diagnose(p, ppl, n_infected, pop_prevalence, i)
    ...
    m = update_output(o, p, m, i, "n_mass_tested", p$number_mass_test[i])
...
initiate_ppl = function(o, p) {
  ...
  ppl[, vaccine_priority  := "none"]  # Assigned below
  ...
  # Sanity check that all risk groups are being considered
  if (!all(p$risk_groups$risk_group %in% names(ppl)))
    stop("Unrecognised risk groups - see ", 
         substring(str_remove(o$pth$risk_pop, o$pth$code), 2))
  ...
  for (i in seq_len(nrow(p$risk_groups))) {
    ...
    risk_prop = c(this_group$age_prob, 1 - this_group$age_prob)
    
    # Sample TRUE and FALSE for all in this age group accordingly 
    ppl[age >= this_group$lower_age & age <= this_group$upper_age, 
        (this_group$risk_group) := sample_vec(c(TRUE, FALSE), 
                                              size = .N,
                                              prob = risk_prop, 
                                              replace = TRUE)]
  }
  ...
  # We need to parse mass testing parameters - could be cleaner
  p = parse_parameters(o, p)
  
  # Number of modelled people willing to be regularly mass tested
  n_mass_test_accept = round(p$population_size * p$mass_test_acceptance)
  ...
  accept_id = wrswoR::sample_int_crank(n = p$population_size,
                                       ...
                                       prob = p$mass_testing_prob[ppl[, age] + 1])
...
# ---------------------------------------------------------
# Parse several key model input parameters
# ---------------------------------------------------------
parse_parameters = function(o, p, network = NULL) {
  
  # NOTE: This parsing is performed here rather than in get_parameters to improve 
  #       accessability to key model inputs (eg mass_testing_dist, ...)
  
  # ---- Mass testing age-distribution ----
  
  # Convert distribution string into a function
  fn_mass_testing = eval(parse(text = paste0('function(x) {return(', p$mass_testing_dist, ')}')))
  
  # Evaluate at normalised age points
  mass_testing_age = fn_mass_testing((o$all_ages + 1) / max(o$all_ages + 1))
  
  # Normalise probabilities and store (mormalisation not strictly necessary here)
  p$mass_testing_prob = mass_testing_age / sum(mass_testing_age)
  ...
  p$number_mass_test = round(p$number_mass_test / p$population_scaler)
...
format_output = function(o, p, m, run_time) {
  ...
  m[metric == "npi_effect", value := npi_level]
...
fn_prognosis = function(o, p, new_infected, date_idx) {
  ...
  p_asym = rep(p$proportion_asymptomatic, nrow(o$disease_df))
  ...
  symptom_severe = pmin(o$disease_df$symptom_severe * p$age_correction, 1)
  ...
  # 
  # NOTE: Improved care practices and triage - represented by improved_care_scaler - can alter this 
  severe_critical = pmin(o$disease_df$severe_critical * p$improved_care_scaler[date_idx], 1)
  ...
  # Age-dependant hospital-seeking behaviour scaler
  seek_hospital = p$seek_hospital * o$disease_df$seek_factor
  
  # Severe but non-critical by age-dependant hospital-seeking behaviour 
  ...
  p_seek_C = p_critical * seek_hospital * (1 - p$critical_death_icu * o$disease_df$death_factor)
  p_seek_D = p_critical * seek_hospital * p$critical_death_icu * o$disease_df$death_factor
  
  # Of the critical cases outside of the care system, who will recover and who will die
  p_noseek_C = p_critical * (1 - seek_hospital) * (1 - p$critical_death_non_icu * o$disease_df$death_factor)
  p_noseek_D = p_critical * (1 - seek_hospital) * p$critical_death_non_icu * o$disease_df$death_factor
...
# ---------------------------------------------------------
fn_test_diagnose = function(p, ppl, n_infected, pop_prevalence, date_idx) {
  ...
  # Number of people to diganose through mass testing on this date
  n_diagnose_mass = ceiling(p$number_mass_test[date_idx] * pop_prevalence / 100)
  
  # Skip this step if there is no one else to diagnose
  if (n_diagnose_mass > 0) {
    
    # All people who could potentially be diagnosed through mass testing today
    mass_potential = ppl[mass_test_accept == TRUE & viral_load > 0 &
                           diagnosed == FALSE & !(id %in% diagnosed_id), ]
    
    # Skip this step if there in no one left to diagnose
    if (nrow(mass_potential) > 0) {
      
      # Sample n_diagnose_mass people with age-related probabilities
      mass_id = sample_vec(mass_potential[, id], 
                           size = min(n_diagnose_mass, nrow(mass_potential)),
                           prob = p$mass_testing_prob[mass_potential[, age] + 1])
      
      # hist(ppl[mass_id, age])
      
      # Concatenate with any guaranteed diagnoses
      diagnosed_id = sort(c(diagnosed_id, mass_id))
    }
  }
  
  # ---- Prepare output ----
  
  # Sanity check that we aren't diagnosing susceptibles
  if ("susc" %in% unique(ppl[diagnosed_id, disease_state]))
    stop("It is an error to diagnose susceptibles")
  
  # Calculate diagnosis ratio - this is what we'll use to generate future assumptions
  diagnosed_ratio = length(diagnosed_id) / n_infected
...
fn_immunity = function(p, ppl) {
  ...
  # TODO: Move acquired immunity code here too (see days_recovered in ppl)
