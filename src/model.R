...
model = function(o, scenario, seed = NA, fit = NULL, do_plot = FALSE, verbose = "date") {
  ...
  if (verbose != "none") message(paste0(" - Running model for scenario '", p$.id, "'"))
  ...
  # ---- Handle manuscript-specific cases first ----
  
  # Case 1: Teacher vaccination scenario
  
  # Layer structure is set to age, yet teacher sampling should be done differently
  if(p$network_structure == "age" && p$overwrite_teacher_sampling == TRUE) {
    
    # Extract number of students based on age groups specified for school layer
    n_students <- ppl_df[age >= p$layer_properties$school$age_min & age <= p$layer_properties$school$age_max, .N]
    
    # Based on number of students, how many classes are in network
    n_classes <- ceiling(n_students/p$layer_properties$school$cluster_sampling$value)
    
    # IDs of potential teachers
    idx_adults <- ppl_df[age >= p$layer_properties$workplace$age_min & age <= p$layer_properties$workplace$age_max, id]
    
    # Assign teacher status randomly
    if(length(idx_adults) >= n_classes) 
      ppl_df[sample_vec(idx_adults, n_classes, replace = FALSE), teacher := TRUE]
    
    # Reset vaccination priority (to reflect teacher status)
    ppl_df <- assign_vaccination_priority(o, p, ppl_df)
    
  }
  
  # Replace beta by re-calculated community beta
  if(p$network_structure == "layers") {
    
    # Is any beta_scaler different from 1?
    if(any(unname(unlist(p$layer_properties$beta_scaler)) != 1)) {
      
      scalers <- p$layer_properties$beta_scaler
      
      contact_structure <- network$edge_list[, .N, by = .(layer)]
      
      contacts_total <- sum(contact_structure$N)
      
      missing_layers <- setdiff(p$layers_simulated, contact_structure$layer)
      
      if(length(missing_layers) > 0)
        contact_structure <- rbindlist(list(contact_structure, list(layer = missing_layers, N = 0)))
      
      # All layers are included in simulation
      if(all(c("household", "school", "workplace") %in% p$layers_simulated)) {
        
        p$beta <- p$beta*(contact_structure[layer == "household", N]/contacts_total * scalers$household +
                            contact_structure[layer == "school", N]/contacts_total * scalers$school + 
                            contact_structure[layer == "workplace", N]/contacts_total * scalers$workplace +
                            contact_structure[layer == "community", N]/contacts_total)^(-1)
      
      } else if("household" %in% p$layers_simulated) {
  ...
  # Reset vaccination priority (teachers are only assigned during network creation)
  if(p$network_structure == "layers" && "school" %in% p$network_layers) {
    
    # Isolate all people who have contacts within school layer
    school_members <- network$edge_list[layer == "school", from]
    
    # Filter for working age population
    all_adults <- ppl_df[age >= p$layer_properties$workplace$age_min & age <= p$layer_properties$workplace$age_max, id]
    
    # Filter for adults who have contacts within school layer
    teachers <- intersect(all_adults, school_members)
    
    # Reset teacher status in people df (was randomly allocated before)
    ppl_df[, teacher := FALSE]

    if(p$overwrite_teacher_sampling == FALSE) {
      
      # Assign teacher status to correct people
      ppl_df[id %in% teachers, teacher := TRUE]
      
    } else if(p$overwrite_teacher_sampling == TRUE) {
      
      # Assign teacher status to random adults
      ppl_df[sample_vec(all_adults, length(teachers), replace = FALSE), teacher := TRUE]
    ...
    # Reallocate vaccination priority to people
    ppl_df <- assign_vaccination_priority(o, p, ppl_df)
  ...
  for (i in 1 : p$n_days) {
    ...
    # ---- Resample community contacts at every single point in time ----
    if(p$community_sampling == "dynamic") { 
      
      network$edge_list <- network$edge_list[layer != "community",]
      
      if(p$network_structure == "layers") {
        
        ppl[, age_group := cut(age, breaks = c(-Inf, network$enhanced_p$network$age_breaks), labels = FALSE)]
        
        network$edge_list <- create_residual(o, network$enhanced_p, ppl, network$edge_list)
        
      } else if(p$network_structure == "age") {

        network$edge_list <- create_age(o, p, ppl)
        
      }
    }
    
    # NPI effect for today on contact reduction in each layer
    npi_df = p$npi_effect[date == i, .(layer, npi_effect)]
    
    # ---- Introduce workplace sampling overwrite ----
    
    # Hardcoded workplace closure - remapping of workplace contacts and temporarily removing them
    if(p$network_structure == "age" && p$overwrite_workplace_sampling == TRUE && i > 1) {
      
      # Filter out npi_df from previous day
      npi_df_previous = p$npi_effect[date == (i-1), .(layer, npi_effect)]  
      
      if((npi_df[layer == "workplace", npi_effect] != npi_df_previous[layer == "workplace", npi_effect])) {
        
        if(npi_df[layer == "workplace", npi_effect] == 1) {
          
          temp_edge_list <- copy(network$edge_list)
          
        } else {
          
          # Temporarily add indicator for working population (based on layer properties)
          ppl[, "working_population" := FALSE]
          
          # Set indicator for working_population if age is within range specified in layer properties
          ppl[age >= p$layer_properties$workplace$age_min & age <= p$layer_properties$workplace$age_max, "working_population" := TRUE]
          
          # Calculate number of teachers based on average class size
          n_teachers <- round(ppl[age >= p$layer_properties$school$age_min & age <= p$layer_properties$school$age_max, .N]/p$layer_properties$school$cluster_sampling$value)
          
          # Randomly sample teacher IDs and exclude from pool for workplace layer
          ppl[id %in% sample_vec(ppl[working_population == TRUE, id], size = n_teachers, replace = FALSE), 
              working_population := FALSE]
          
          # Calculate hypothetical number of workplace contacts
          n_contacts_wp <- ppl[working_population == TRUE, .N] * p$layer_properties$workplace$contacts / 2
          
          # Check who could be in working population
          temp_edge_list <- copy(network$edge_list)
          
          temp_edge_list <- temp_edge_list %>%
            left_join(ppl[, .(id,working_population)], by = c("from" = "id")) %>%
            rename(from_wp = working_population) %>%
            left_join(ppl[, .(id, working_population)], by = c("to" = "id")) %>%
            rename(to_wp = working_population) %>%
            filter(from < to) %>%
            mutate(both_wp = ifelse(from_wp == TRUE & to_wp == TRUE, TRUE, FALSE))
          
          # Remove flag for working population
          ppl[, working_population := NULL]
          
          # Append row number, necessary for filtering
          temp_edge_list[, i := .I]
          
          # Filter row numbers of contacts who COULD be workplace contacts
          idx_working_contacts <- temp_edge_list[both_wp == TRUE, i]
          
          # Temporarily assign all contacts to community
          temp_edge_list[, layer := "community"]
          
          # Catch if more contacts than defined should be deleted
          if(n_contacts_wp > length(idx_working_contacts)) 
            n_contacts_wp <- length(idx_working_contacts)
          
          # Re-map newly assigned contacts to workplace; all others remain in community
          temp_edge_list[i %in% sample_vec(idx_working_contacts, n_contacts_wp, replace = FALSE), layer := "workplace"]
          
          # Convert back to two-sided edge list
          temp_edge_list <- rbindlist(list(temp_edge_list,
                                           temp_edge_list %>% mutate(to = temp_edge_list$from, 
                                                                     from = temp_edge_list$to))) %>%
            select(from, to, layer)
      ...
      # If temporary edge list doesn't yet exist, take the originally sampled one (probably occuring on day 2)
      if(!exists("temp_edge_list"))
        temp_edge_list <- copy(network$edge_list)

      # Calculate infectious contacts based on temporary edge list
      infectious_contacts = temp_edge_list %>%
        left_join(npi_df, by = "layer") %>%
        mutate(rand = runif(n()), 
               meet = rand < npi_effect) %>%
        filter(meet == TRUE) %>%
        select(from, to, layer) %>%
      ...
      #### If workplace contacts should not be overwritten, run this:
    ...
    # Scale beta according to layer specification (if network structure selected)
    if(p$network_structure == "layers") {
      
      beta <- beta * 
        as.numeric(p$layer_properties$beta_scaler[infectious_contacts$layer])
    }      
    ...
    transmission_df = immunity_contacts %>% 
      ...
      slice_head(n = 1) %>% # Only consider first transmission if multiple
      mutate(variant = ppl[from, variant]) %>%  # Variant is passed down from infector
      select(from, to, variant, layer) %>%
      as.data.table()
    ...
    # Extract the IDs, variants, and source layers of new local transmissions
    local_id      = transmission_df$to 
    apply_variant = transmission_df$variant
    source_layer  = transmission_df$layer
    
    # Begin infections for these people
    ppl = fn_begin_infection(o, p, ppl, local_id, apply_variant, source_layer)
    ...
    # Begin infections for these people
    ppl = fn_begin_infection(o, p, ppl, import_id, apply_variant, "community")
    ...
    if (i %in% p$variants$import_day) {
      ...
      if (n_imports_remain > 0) {
        ...
        # Begin infections for these people
        ppl = fn_begin_infection(o, p, ppl, newly_forced_id, this_variant$id, "community")
...
initiate_ppl = function(o, p, verbose) {
  ...
  ppl = data.table(	
    ...
    teacher           = logical(p$population_size),    # Is this individual working in educational facility
    ...
    layer             = character(p$population_size),  # On which network layer did the transmission happen
  ...
  ppl[, teacher           := FALSE]   # Assigned below
  ...
  # Step sourced out to separate function so that it can be called after teacher status was allocated in network construction
  ppl <- assign_vaccination_priority(o, p, ppl)
...
# ---------------------------------------------------------
# Assign vaccination priority groups
# ---------------------------------------------------------
assign_vaccination_priority <- function(o, p, ppl) {
...
initiate_epidemic = function(o, p, ppl) {
  ...
  # Begin infections for these people
  ppl = fn_begin_infection(o, p, ppl, prev_infected_id, p$variants$id[1], "community")
...
# ---------------------------------------------------------
# Prepare final model output (several special cases to deal with)
# ---------------------------------------------------------
format_output = function(o, p, m, R_info, network, ppl, yaml, seed) {
  ...
  # Do calculation for contact_reduction only if collecting this metric
  if ("contact_reduction" %in% p$metrics$df$metric) {
    
    # Vector of NPI effect in format of output datatable
    npi_effect = m[metric == "contact_reduction", ] %>%
      left_join(p$npi_effect, by = c("date", "group" = "layer")) %>%
      # replace_na(list(npi_effect = 0)) %>%
      pull(npi_effect)
    
    # Populate output datatable with this NPI effect vector
    m[metric == "contact_reduction", value := npi_effect]
  }
  
  # Do calculation for average_beta only if collecting this metric
  if ("average_beta" %in% p$metrics$df$metric) {
    
    # Extract beta scalers from layer_properties
    beta_df = as.data.table(p$layer_properties$beta_scaler) %>%
      pivot_longer(cols = everything(), 
                   names_to  = "layer", 
                   values_to = "beta_scaler") %>%
      mutate(beta = p$beta * beta_scaler)
    
    # Join beta according to layer of contact
    all_beta = network$edge_list %>%
      left_join(beta_df, by = "layer") %>%
      pull(beta)
    
    # Store the average beta of the network
    m[metric == "average_beta", value := mean(all_beta)]
  }
...
# ---------------------------------------------------------
fn_begin_infection = function(o, p, ppl, id, apply_variant, source_layer) {
  ...
  # Save layer on which infectious contact occurred
  if (length(source_layer) > 0)
    ppl[id, layer := source_layer]
...
fn_prognosis = function(o, p, new_infected) {
  ...
  disease_prevention = 1 - probability_symptoms - p$proportion_asymptomatic
