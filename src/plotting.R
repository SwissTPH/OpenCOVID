...
# ---------------------------------------------------------
plot_temporal = function(o, canton, fig_name, plot_file = NULL, likelihood = NA, 
                         plot_geom = "line", ...) {
  ...
  # data_df = filter(data_df, metric != "seroprevalence")
  ...
  } else if (plot_geom == "area") {
    g = g + geom_area()
    
    # Normalised version of the are plot
  } else if (plot_geom == "area_norm") {
    
    stop("Feature under construction")
  }
  
  # Throw an error if any other value is provided
  else {
    stop("Value of plot_geom '", plot_geom, "' not recognised")
  }
...
plot_posteriors = function(o, n_bins = 20) {
  ...
  for (canton in o$cantons) {
    ...
    fit = try_load(o$pth$fitting, paste0(canton, "_calibration"), msg = err_msg) 
...
output_csv = function(o, canton, file_name, bounds = TRUE, ...) {
  ...
  f = figure_properties(o, scenario_groupings_ok = TRUE, override_colours = NA, ...)
