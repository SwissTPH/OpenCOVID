...
# ---------------------------------------------------------
format_risk_groups = function(o) {
  
  # Load risk group properties from file and parse
  o$risk_groups = read.csv(o$pth$risk_pop) %>%
    mutate(lower_age = ifelse(is.na(lower_age), min(o$all_ages), lower_age), 
           upper_age = ifelse(is.na(upper_age), max(o$all_ages), upper_age))
...
# ---------------------------------------------------------
format_disease_probabilities = function(o) {
  
  # Load variant properties file
  o$disease_df = read.csv(o$pth$disease) %>%
    mutate(symptom_severe  = symptom_severe / 100, 
           severe_critical = severe_critical / 100, 
           death_factor    = death_factor / 100)
